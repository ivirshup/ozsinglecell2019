...
all_annotations = pandas.read_csv(
    "/Users/pwangel/Data/Metadata_dumps/blood_atlas_annotations_180719.tsv", sep="\t"
).set_index("sample_id")
...
all_annotations.index = numpy.core.defchararray.add(
    all_annotations.index.values.astype(str),
    numpy.full(shape=all_annotations.shape[0], fill_value=";").astype(str),
)
all_annotations.index = numpy.core.defchararray.add(
    all_annotations.index.values.astype(str),
    all_annotations["dataset_id"].values.astype(str),
)

s4m_metadata = all_s4m_metadata.merge(
    all_annotations[["display_metadata", "tier1", "tier2", "tier3", "celltype"]],
    how="inner",
    left_index=True,
    right_index=True,
)
...
blood_annotations = s4m_metadata.loc[
    numpy.in1d(
        s4m_metadata.index.values.astype(str),
        all_annotations.index.values.astype(str)
    )
]

# Select only the blood data, and drop genes that are not measured in all datasets
blood_data = all_data.iloc[
    :,
    numpy.in1d(
        all_data.columns.values.astype(str),
        blood_annotations.index.values.astype(str)
    ),
].dropna(how="any")
...
varPart = variancePartition.fitExtractVarPartModel(
    transform_to_percentile(blood_data), form, blood_annotations[["Platform_Category"]]
)

sel_varPart = (
    numpy.array(varPart)[0] <= 0.2
)  # This is the thresholding, at a value of 0.2
